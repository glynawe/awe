...
▸ In section 8.1, Standard Transfer Functions, remove all mention of `BASE16` and
  `LONGBASE16`.
...
Awe recognises two compiler directives: the `@awe_text` card causes
...
The identifier has no effect, but is usually used to mark the end of a
procedure.
...
in a program compiled by Cygwin GCC. The Cygwin version of Awe uses
...
**Redirect output to stderr**
  ...
  program's output to be temporarily redirected to `stderr` to print
...
See the [Arrays](#arrays) section below for an explanation of how the 
